  ...
  class _BrokerManagementScreenState extends State<BrokerManagementScreen> {
    ...
    void _showBrokerDetailsDialog(BuildContext context, Map<String, dynamic> broker) {
      ...
      final brnIssueDateController = TextEditingController(
        text: brnIssueDate != null
            ? DateFormat('dd-MMM-yyyy')
            .format(DateTime.tryParse(brnIssueDate) ?? DateTime.now())
            : '',
      );

      final brnExpiryDateController = TextEditingController(
        text: brnExpiryDate != null
            ? DateFormat('dd-MMM-yyyy')
            .format(DateTime.tryParse(brnExpiryDate) ?? DateTime.now())
            : '',
      );
      ...
      final hasDetails =
          (company != null && company.toString().trim().isNotEmpty) ||
              (brnNumber != null && brnNumber.toString().trim().isNotEmpty) ||
              (licenseNumber != null && licenseNumber.toString().trim().isNotEmpty) ||
              (reraNumber != null && reraNumber.toString().trim().isNotEmpty);
      ...
      TextEditingController(text: company?.toString() ?? '');

      final licenseController =
      TextEditingController(text: licenseNumber?.toString() ?? '');

      final reraController =
      TextEditingController(text: reraNumber?.toString() ?? '');

      final brnController =
      TextEditingController(text: brnNumber?.toString() ?? '');

      Future<void> _updateBrokerDetails(
      ...
      async {
        ...
        final issueDateForApi = brnIssueDateController.text.isNotEmpty
            ? DateFormat('yyyy-MM-dd').format(
          DateFormat('dd-MMM-yyyy')
              .parse(brnIssueDateController.text),
        )
            : null;

        final expiryDateForApi = brnExpiryDateController.text.isNotEmpty
            ? DateFormat('yyyy-MM-dd').format(
          DateFormat('dd-MMM-yyyy')
              .parse(brnExpiryDateController.text),
        )
            : null;
        ...
        try {
          ...
          final body = jsonEncode({
            ...
            "reraNumber": rera,
            "brnNumber": brn,
            "brn_issues_date": issueDateForApi,
            "brn_expiry_date": expiryDateForApi,
          });
                          ...
                          children: [
                            ...
                            if (hasDetails)
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  _buildEditableField("🏢 Company Name", companyController),
                                  ...
                                  // 🔥 HERE — CALL DATE FIELDS
                                  _buildDateField("📅 BRN Issue Date", brnIssueDateController, context),
                                  const SizedBox(height: 12),

                                  _buildDateField("⏳ BRN Expiry Date", brnExpiryDateController, context),
                                  const SizedBox(height: 16),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: () async {
                                  await _updateBrokerDetails(
                                    context,
                                    broker['id'],
                                    companyController.text.trim(),
                                    licenseController.text.trim(),
                                    reraController.text.trim(),
                                    brnController.text.trim(),
                                  );
                                },
                                child: Text(
                                  "Update",
