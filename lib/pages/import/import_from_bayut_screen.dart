...
class _ImportFromBayutScreenState extends State<ImportFromBayutScreen>
  ...
  Future<void> _importBayutData() async {
    if (bayutUrlC.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text("Please enter a Bayut property URL."),
          backgroundColor: Colors.redAccent,
          behavior: SnackBarBehavior.floating,
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({"url": bayutUrlC.text.trim()}),
      );
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            // 🟢 Import Data button
            ElevatedButton.icon(
              onPressed: _isImporting ? null : _importBayutData,
              icon: _isImporting
                  ? const SizedBox(
                height: 18,
                width: 18,
